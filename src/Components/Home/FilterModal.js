...
const FilterModal = ({selectedFilters,onFilterChange,onClose}) => {
    ...
    const [propertyType, setPropertyType]=useState(
        selectedFilters.propertyType || "" //default is is empty or selected property type from props
    )
    ...
    // useeffect hook to update states when selectefilters prop changes
    useEffect(() =>{
        setPriceRange({
            min: selectedFilters.priceRange?.min || 600,
            max: selectedFilters.priceRange?.max || 30000,
    ...
    const handlePriceRangeChange =(value) =>{
        setPriceRange(value) //it will update price range state
    }
